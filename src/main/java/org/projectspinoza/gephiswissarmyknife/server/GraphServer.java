...
public class GraphServer {
  ...
  private void deployGsakRoutes() {
     ...
     * 
     * */
    router.getWithRegex("/demo.*").method(HttpMethod.GET).handler(routingContext -> {
      dtoConfig.setDataSource("inputfile");
      dtoConfig.setTextfileName(routingContext.request().getParam("demoselectId"));
      dtoConfig.setSearchValue(routingContext.request().getParam("demosubmitsearch").replaceAll(",", ""));
      this.graphGen.setGraphModel(gephiGraphWs.getGraphModel());
      this.gephiGraph = this.graphGen.createGraph();
      responseSigmaGraph(this.gephiGraph, routingContext, true);
      this.graphBackup.saveGraph(this.gephiGraph, false);
