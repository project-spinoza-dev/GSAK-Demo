...
//initialize server for proxy use
requestAjax ("./proxy/proxy.php", {"route" : "proxyPageOnLoad"}, function(resp){});
      ...
      $('#localGraphSettingsChanger input[type="checkbox"]').each(function() {
            ...
            if (chk_name == "nodesrandomcolors") {
                if ( $('input[name="'+chk_name+'"]').is(':checked') ) {
                  requestAjax ("./proxy/proxy.php", {'autoColor': 'true', "route":"autoColor"}, function (resp) {});
                }else if (!$('input[name="'+chk_name+'"]').is(':checked') ){
                  requestAjax ("./proxy/proxy.php", {'autoColor': 'false', "route":"autoColor"}, function (resp) {});
                }
            }else {
                switch (chk_name) {
                  case "showlabel":
...
$('#colorSelector').ColorPicker({
  ...
  onHide: function (colpkr) {
    ...
    requestAjax ("./proxy/proxy.php", {'edgeColor': rgb[0]+','+rgb[1]+','+rgb[2] , "route":"defaultEdgeColor"}, function (resp) {});
    return false;
  },
...
$(".layout_form").submit(function (e) {
    e.preventDefault();
    $(".graphLoader-run").css('visibility','visible');
    requestAjax ("./proxy/proxy.php", $("#" + this.id).serialize()+"&route=layout", function (graphData) {
      graphJsonHandler(graphData);
      $(".graphLoader-run").css('visibility','hidden');
    });
});
  ...
  e.preventDefault();
    $('#filter_querycontainer .ui-widget-content.ui-selected').find('a').each(function() {
       var selected_filter = $(this).attr('href');
        ...
        requestAjax ("./proxy/proxy.php", $(selected_filter+'_form').serialize()+"&route=selectFilter", function(graphData){
          graphJsonHandler(graphData);
...
function filterGraph(filter){
  requestAjax ("./proxy/proxy.php", $(filter+'_form').serialize()+"&route=filterGraph", function(graphData) {
...
$('#filterUndo').on('click',function(e){
  $('.filterLoader').css('display','block');
  e.preventDefault();
  requestAjax ("./proxy/proxy.php", {"route":"unselectFilter"}, function(graphData){
    graphJsonHandler(graphData);
    $('.filterLoader').css('display','none');
...
//var temp = $("#" + this.id).serialize();
 //  alert(temp);
    ...
    requestAjax ("./proxy/proxy.php", $("#" + this.id).serialize()+"&route=statistics", graphStatisticsHandler);
});
...
$("#graphFileUploadForm").submit(function (e) {
  ...
  requestAjax ("./proxy/proxy.php", {"route":"extractGraph"}, function(graphData) {
    graphJsonHandler(graphData);
    $(".popup-close").click();
...
$("#original_graph_load_form").submit(function (e) {
  ...
  requestAjax ("./proxy/proxy.php", {"route":"originalGraph"}, function(graphData) {
    graphJsonHandler(graphData);
...
$("#mysqldbForm").submit(function (e) {
  $('.dbLoader').css('visibility','visible');
  e.preventDefault();
  requestAjax ("./proxy/proxy.php", $("#" + this.id).serialize()+"&route=connectDB", function (resp) {
...
$("#search-form").submit(function (e) {
  ...
  requestAjax ("./proxy/proxy.php","searchStr="+searchVal+"&datasource="+datasource+"&route=search", function(graphData) {
    $("img#searchLoader").css('visibility','hidden');
    graphJsonHandler(graphData);
    getDegreeRanges();
  });
...
function getDegreeRanges(){
  requestAjax ("./proxy/proxy.php", {"route":"filterRanges"}, function (ranges){
...
function graphStatisticsHandler (statsData, formData){  
  var statistics_id = formData.split('=')[1].split('&')[0]; //get hidden field ID
